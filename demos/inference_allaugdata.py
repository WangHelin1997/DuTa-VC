...
def inference(args, generator, hifigan_universal, src_path, tgt_path, save_path, mean, std, emb):
    ...
    phoneme_logits = np.load(os.path.join(args.phoneme_dir, src_path.split('/')[-2], src_path.split('/')[-1].split('.')[0]+'_phonemes.npy'))
...
def main(args, dys):
    ...
    print(len(cmds))
    for root, dir, files in os.walk(os.path.join(args.test_dir, dys)):
        for f in files:
            if f.endswith('.wav'):
    ...
    for c in tqdm(cmds):
        tag = os.path.join(args.test_dir, dys, dys+'_'+c.split('/')[-1].split('_',1)[-1])
        tgt_path = target_cmds[0]
        if tag in target_cmds:
            tgt_path = tag
        inference(args, generator, hifigan_universal, src_path=c, tgt_path=tgt_path, save_path=results_dir, mean=mean, std=std, emb=allembs)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--results_dir', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/results_allaugdata_removesil')

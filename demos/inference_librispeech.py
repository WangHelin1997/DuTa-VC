...
def inference(args, generator, hifigan_universal, src_path, tgt_path, save_path, mean, std, emb):
    with open(args.phoneme_uaspeech, 'rb') as f:
    ...
    phoneme_logits = np.load(os.path.join(args.phoneme_dir_ls, src_path.split('/')[-1].split('.')[0]+'_phonemes.npy'))
...
def main(args, dys, cmds):
    stats = np.loadtxt(os.path.join(args.mean_std_file, 'global_mean_var.txt'))
    mean = stats[0]
    std = stats[1]
    ...
    for c in tqdm(cmds):
        inference(args, generator, hifigan_universal, src_path=c, tgt_path=c, save_path=results_dir, mean=mean, std=std, emb=allembs)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--results_dir', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/results_allaugdata_librispeech')
    parser.add_argument('--model_path_dir', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/logs_dec_alldys')
    parser.add_argument('--vocoder_dir', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/hifi-gan/checkpoints')
    parser.add_argument('--pkl_path', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/librispeechData/mels_mode.pkl')
    parser.add_argument('--pt_path', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/librispeechData/phonemes.pkl')
    parser.add_argument('--enc_dir', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/logs_enc_LT')
    parser.add_argument('--test_dir', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/mfa_data')
    parser.add_argument('--test_dir_ls', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/librispeechData')
    parser.add_argument('--phoneme_dir', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/avgmel_data/phonemes')
    parser.add_argument('--phoneme_dir_ls', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/librispeechData/phonemes')
    parser.add_argument('--melmode_dir', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/avgmel_data/mels_mode')
    parser.add_argument('--mean_std_file', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/librispeechData/')
    parser.add_argument('--mean_std_file_ua', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/avgmel_data/')
    parser.add_argument('--phoneme_librispeech', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/librispeechData/phonemes.pkl')
    parser.add_argument('--phoneme_uaspeech', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/avgmel_data/M16_phonemes.pkl')
    parser.add_argument('--emb_dir', type=str,
                        default='/data/dean/whl-2022/Speech-Backbones/DiffVC/avgmel_data/embeds')
    parser.add_argument('--slow_ratio', type=float, default=1.2)
    parser.add_argument('--fast_ratio', type=float, default=0.8)
    parser.add_argument('--spk', type=str, default='F02')
    parser.add_argument('--debug', action='store_true')
    parser.set_defaults(debug=False)
    args = parser.parse_args()
    # main(args, args.spk)
    cmds = []
    for root, dir, files in os.walk(args.test_dir_ls):
        for f in files:
            if f.endswith('.flac'):
    ...
    if args.debug:
        cmds = cmds[:10]
    os.makedirs(os.path.join(args.results_dir, 'source'), exist_ok=True)
    for c in cmds:
        shutil.copyfile(c, os.path.join(args.results_dir, 'source', c.split('/')[-1]))
    alldys = ['F02', 'F05', 'M04', 'M05', 'M09', 'M16']
    for dys in alldys:
        main(args, dys, cmds)